  ...
  static func spawnAndRun(name: String? = nil, body: @escaping (Thread) -> Void) {
    ...
    let res = pthread_create(
      ...
      nil,
      {
        ...
        let b =
          Unmanaged<ThreadBox>.fromOpaque((p as UnsafeMutableRawPointer?)!).takeRetainedValue()
...
public struct ThreadSpecificVariable<T: AnyObject> {
  ...
  public var currentValue: T? {
    ...
    set {
      ...
      let pthreadErr = pthread_setspecific(
        self.key,
        newValue.map { v -> UnsafeMutableRawPointer in
