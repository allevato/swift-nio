...
public struct CircularBuffer<E>: CustomStringConvertible, AppendableCollection {
  ...
  private(set)  /* private but tests */
  internal var headIdx = 0

  /// The index into the buffer of the next free slot
  private(set)  /* private but tests */
  internal var tailIdx = 0
  ...
  private mutating func doubleCapacity() {
    ...
    let newCapacity =
      self.buffer.count << 1  // Double the storage.
  ...
  public mutating func removeAll(keepingCapacity: Bool = false) {
    ...
    self.buffer = ContiguousArray<E?>(
      repeating: nil,
      count: keepingCapacity ? self.buffer.count : 1)
  }
  ...
  RangeReplaceableCollection
{
  ...
  where C: Collection, E == C.Element {
    precondition(
    ...
    if subrange.count == newElements.count {
      ...
      let neededNewCapacity =
        self.count + newElements.count - subrange.count + 1  /* always one spare */
  ...
  public mutating func removeSubrange(_ bounds: Range<Int>) {
    precondition(
      bounds.upperBound >= self.startIndex && bounds.upperBound <= self.endIndex,
      "Invalid bounds.")
    switch bounds.count {
    case 1:
  ...
  public mutating func removeLast(_ n: Int) {
    precondition(
      n <= self.count,
      "Number of elements to drop bigger than the amount of elements in the buffer.")
  ...
  @discardableResult public mutating func remove(at position: Int) -> E {
