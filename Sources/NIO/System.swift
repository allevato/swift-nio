...
//
  ...
  let getifaddrs: @convention(c)
  (UnsafeMutablePointer<UnsafeMutablePointer<ifaddrs>?>?) -> CInt = android_getifaddrs
  let freeifaddrs: @convention(c)
  (UnsafeMutablePointer<ifaddrs>?) -> Void = android_freeifaddrs
...
private let sysClose: @convention(c)
(CInt) -> CInt = close
private let sysShutdown: @convention(c)
(CInt, CInt) -> CInt = shutdown
private let sysBind: @convention(c)
(CInt, UnsafePointer<sockaddr>?, socklen_t) -> CInt = bind
private let sysFcntl: @convention(c)
(CInt, CInt, CInt) -> CInt = fcntl
private let sysSocket: @convention(c)
(CInt, CInt, CInt) -> CInt = socket
private let sysSetsockopt: @convention(c)
(CInt, CInt, CInt, UnsafeRawPointer?, socklen_t) -> CInt = setsockopt
private let sysGetsockopt: @convention(c)
(CInt, CInt, CInt, UnsafeMutableRawPointer?, UnsafeMutablePointer<socklen_t>?) -> CInt = getsockopt
private let sysListen: @convention(c)
(CInt, CInt) -> CInt = listen
private let sysAccept: @convention(c)
(CInt, UnsafeMutablePointer<sockaddr>?, UnsafeMutablePointer<socklen_t>?) -> CInt = accept
private let sysConnect: @convention(c)
(CInt, UnsafePointer<sockaddr>?, socklen_t) -> CInt = connect
private let sysOpen: @convention(c)
(UnsafePointer<CChar>, CInt) -> CInt = open
private let sysOpenWithMode: @convention(c)
(UnsafePointer<CChar>, CInt, mode_t) -> CInt = open
private let sysWrite: @convention(c)
(CInt, UnsafeRawPointer?, CLong) -> CLong = write
private let sysRead: @convention(c)
(CInt, UnsafeMutableRawPointer?, CLong) -> CLong = read
private let sysLseek: @convention(c)
(CInt, off_t, CInt) -> off_t = lseek
#if os(Android)
...
#else
  private let sysRecvFrom: @convention(c)
  (
    CInt,
  ...
  private let sysWritev: @convention(c)
  (Int32, UnsafePointer<iovec>?, CInt) -> CLong = writev
#endif
private let sysSendTo: @convention(c)
(CInt, UnsafeRawPointer?, CLong, CInt, UnsafePointer<sockaddr>?, socklen_t) -> CLong = sendto
private let sysDup: @convention(c)
(CInt) -> CInt = dup
private let sysGetpeername: @convention(c)
(CInt, UnsafeMutablePointer<sockaddr>?, UnsafeMutablePointer<socklen_t>?) -> CInt = getpeername
private let sysGetsockname: @convention(c)
(CInt, UnsafeMutablePointer<sockaddr>?, UnsafeMutablePointer<socklen_t>?) -> CInt = getsockname
private let sysGetifaddrs: @convention(c)
(UnsafeMutablePointer<UnsafeMutablePointer<ifaddrs>?>?) -> CInt = getifaddrs
private let sysFreeifaddrs: @convention(c)
(UnsafeMutablePointer<ifaddrs>?) -> Void = freeifaddrs
private let sysIfNameToIndex: @convention(c)
(UnsafePointer<CChar>?) -> CUnsignedInt = if_nametoindex
private let sysAF_INET = AF_INET
private let sysAF_INET6 = AF_INET6
private let sysAF_UNIX = AF_UNIX
private let sysInet_ntop: @convention(c)
(CInt, UnsafeRawPointer?, UnsafeMutablePointer<CChar>?, socklen_t) -> UnsafePointer<CChar>? =
  inet_ntop

#if os(Linux)
  private let sysSendMmsg: @convention(c)
  (CInt, UnsafeMutablePointer<CNIOLinux_mmsghdr>?, CUnsignedInt, CInt) -> CInt = CNIOLinux_sendmmsg
  private let sysRecvMmsg: @convention(c)
  (
    CInt,
    UnsafeMutablePointer<CNIOLinux_mmsghdr>?,
    CUnsignedInt,
    CInt,
    UnsafeMutablePointer<timespec>?
  ) -> CInt = CNIOLinux_recvmmsg
#else
  private let sysKevent = kevent
  private let sysSendMmsg: @convention(c)
  (CInt, UnsafeMutablePointer<CNIODarwin_mmsghdr>?, CUnsignedInt, CInt) -> CInt =
    CNIODarwin_sendmmsg
  private let sysRecvMmsg: @convention(c)
  (
    CInt,
    UnsafeMutablePointer<CNIODarwin_mmsghdr>?,
    CUnsignedInt,
    CInt,
    UnsafeMutablePointer<timespec>?
...
private func assertIsNotBlacklistedErrno(err: CInt, where function: StaticString) {
  // strerror is documented to return "Unknown error: ..." for illegal value so it won't ever fail
  assert(
    !isBlacklistedErrno(err),
    "blacklisted errno \(err) \(String(cString: strerror(err)!)) in \(function))")
}
...
@inline(__always)
internal func wrapSyscallMayBlock<T: FixedWidthInteger>(
...
internal enum Posix {
  ...
  @inline(never)
  public static func connect(descriptor: CInt, addr: UnsafePointer<sockaddr>, size: socklen_t)
  throws -> Bool {
    do {
      try wrapSyscall {
  ...
  @inline(never)
  public static func write(descriptor: CInt, pointer: UnsafeRawPointer, size: Int) throws
  -> IOResult<Int> {
  ...
  @inline(never)
  public static func writev(descriptor: CInt, iovecs: UnsafeBufferPointer<IOVector>) throws
  -> IOResult<Int> {
    return try wrapSyscallMayBlock {
  ...
  @inline(never)
  public static func read(descriptor: CInt, pointer: UnsafeMutableRawPointer, size: size_t) throws
  -> IOResult<Int> {
    return try wrapSyscallMayBlock {
  ...
  @inline(never)
  public static func sendfile(descriptor: CInt, fd: CInt, offset: off_t, count: size_t) throws
  -> IOResult<Int> {
    var written: off_t = 0
    do {
  ...
  @inline(never)
  public static func getifaddrs(_ addrs: UnsafeMutablePointer<UnsafeMutablePointer<ifaddrs>?>)
  throws {
    try wrapSyscall {
      sysGetifaddrs(addrs)
    }
