  ...
  deinit {
    debugOnly {
      if let eventLoop = self.eventLoop as? SelectableEventLoop {
        ...
        if !isFulfilled {
          fatalError(
            "leaking promise created at \(creation)",
            file: creation.file,
            line: creation.line)
        }
      }
...
extension EventLoopFuture {
  ...
  public func then<U>(
    ...
    line: UInt = #line,
    _ callback: @escaping (T) -> EventLoopFuture<U>
  ) -> EventLoopFuture<U> {
  ...
  public func thenThrowing<U>(
    ...
    return self.then(file: file, line: line) { (value: T) -> EventLoopFuture<U> in
      do {
        return EventLoopFuture<U>(
          eventLoop: self.eventLoop,
          result: try callback(value),
          file: file,
          line: line)
      }
      catch {
  ...
  public func thenIfErrorThrowing(
    file: StaticString = #file,
    line: UInt = #line,
    _ callback: @escaping (Error) throws -> T
  ) -> EventLoopFuture<T> {
  ...
  public func thenIfError(
    file: StaticString = #file,
    line: UInt = #line,
    _ callback: @escaping (Error) -> EventLoopFuture<T>
  ) -> EventLoopFuture<T> {
  ...
  ///         a new value lifted into a new `EventLoopFuture`.
  /// - returns: A future that will receive the recovered value.
  public func mapIfError(
    file: StaticString = #file,
    line: UInt = #line,
    _ callback: @escaping (Error) -> T
  ) -> EventLoopFuture<T> {
...
extension EventLoopFuture {
  ...
  public func and<U>(_ other: EventLoopFuture<U>, file: StaticString = #file, line: UInt = #line)
  -> EventLoopFuture<(T, U)> {
  ...
  public func and<U>(result: U, file: StaticString = #file, line: UInt = #line)
  -> EventLoopFuture<(T, U)> {
    return and(
      EventLoopFuture<U>(eventLoop: self.eventLoop, result: result, file: file, line: line))
  }
}
...
"""
      ...
      precondition(
        MultiThreadedEventLoopGroup.currentEventLoop == nil,
        explainer(),
        file: file,
        line: line)
    }
...
extension EventLoopFuture {
  ...
  public func fold<U>(
    _ futures: [EventLoopFuture<U>],
    with combiningFunction: @escaping (T, U) -> EventLoopFuture<T>
  ) -> EventLoopFuture<T> {
    let body = futures.reduce(self) {
      (f1: EventLoopFuture<T>, f2: EventLoopFuture<U>) -> EventLoopFuture<T> in
      let newFuture = f1.and(f2).then {
        (args: (T, U)) -> EventLoopFuture<T> in
...
extension EventLoopFuture {
  ...
  public static func andAll(_ futures: [EventLoopFuture<Void>], eventLoop: EventLoop)
  ...
  public static func reduce<U>(
    _ initialResult: T,
    _ futures: [EventLoopFuture<U>],
    eventLoop: EventLoop,
    _ nextPartialResult: @escaping (T, U) -> T
  ) -> EventLoopFuture<T> {
  ...
  public static func reduce<U>(
    ...
    _ futures: [EventLoopFuture<U>],
    eventLoop: EventLoop,
    _ updateAccumulatingResult: @escaping (inout T, U) -> Void
  ) -> EventLoopFuture<T> {
    let p0 = eventLoop.makePromise(of: T.self)
    var result: T = initialResult

    let f0 = eventLoop.makeSucceededFuture(result: ())
    let future = f0.fold(futures) {
      (_: (), value: U) -> EventLoopFuture<Void> in
