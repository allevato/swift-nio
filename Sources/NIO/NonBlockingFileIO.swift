...
public struct NonBlockingFileIO {
    ...
    do {
      ...
      try fileRegion.fileHandle.withUnsafeFileDescriptor { descriptor in
        _ = try Posix.lseek(
          ...
          offset: off_t(fileRegion.readerIndex),
          whence: SEEK_SET)
      }
      ...
      if remainingReads > 1 || (remainingReads == 1 && lastReadSize > 0) {
        ...
        return
          self.read(
            ...
            byteCount: readSize,
            allocator: allocator,
            eventLoop: eventLoop).then { buffer in
          chunkHandler(buffer).then {
  ...
  public func read(fileRegion: FileRegion, allocator: ByteBufferAllocator, eventLoop: EventLoop)
  ...
  public func read(
    ...
    return self.threadPool.runIfActive(eventLoop: eventLoop) {
      ...
      while bytesRead < byteCount {
        let n = try buf.writeWithUnsafeMutableBytes {
          ptr in
  ...
  public func write(fileHandle: FileHandle, buffer: ByteBuffer, eventLoop: EventLoop)
  -> EventLoopFuture<()> {
    ...
    return self.threadPool.runIfActive(eventLoop: eventLoop) {
      var buf = buffer
      while byteCount > 0 {
        let n = try buf.readWithUnsafeReadableBytes {
          ptr in
          precondition(ptr.count == byteCount)
          let res = try fileHandle.withUnsafeFileDescriptor { descriptor in
            try Posix.write(descriptor: descriptor, pointer: ptr.baseAddress!, size: byteCount)
          }
          switch res {
          case .processed(let n):
  ...
  public func openFile(path: String, eventLoop: EventLoop)
  -> EventLoopFuture<(FileHandle, FileRegion)> {
    return self.threadPool.runIfActive(eventLoop: eventLoop) {
