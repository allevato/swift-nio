...
public protocol ByteToMessageDecoder {
  ...
  mutating func decodeLast(ctx: ChannelHandlerContext, buffer: inout ByteBuffer) throws
  -> DecodingState
...
extension ByteToMessageDecoder {
  ...
  public func decodeLast(ctx: ChannelHandlerContext, buffer: inout ByteBuffer) throws
  -> DecodingState {
    return .needMoreData
  }
...
extension ChannelHandlerContext {
  private func withThrowingToFireErrorAndClose<T>(_ body: () throws -> T) -> T? {
    do {
      return try body()
    }
    catch {
...
extension B2MDBuffer {
  ...
  mutating func finishProcessing(remainder buffer: ByteBuffer) {
...
extension ByteBuffer {
  private mutating func writeBuffers(_ buffers: CircularBuffer<ByteBuffer>) {
...
extension B2MDBuffer {
  private func _testOnlyOneBuffer() -> ByteBuffer? {
    if buffers.count == 0 {
      return nil
    }
...
extension ByteToMessageHandler {
  private func withNextBuffer(_ body: (inout Decoder, inout ByteBuffer) throws -> DecodingState)
  ...
  private func decodeLoop(ctx: ChannelHandlerContext, decodeMode: DecodeMode) throws
  -> B2MDBuffer.BufferProcessingResult {
    while true {
          ...
          case .done:
            ...
            case .leftoversNeedProcessing:
              ...
              case .didProcess:
                ...
                case .cannotProcessReentrantly:
                  preconditionFailure(
                    "bug in NIO: non-reentrant decode loop couldn't run \(self), \(self.state)")
                }
            }
      case .cannotProcessReentrantly:
