...
private func doPendingWriteVectorOperation(
  ...
  assert(
    iovecs.count >= Socket.writevLimitIOVectors,
    "Insufficiently sized buffer for a maximal writev")
  ...
  loop: for i in 0..<count {
    ...
    case .byteBuffer(let buffer):
      ...
      guard (numberOfUsedStorageSlots == 0) || (
          Socket.writevLimitBytes - toWrite >= buffer.readableBytes
        )
      else {
        break loop
      }
      ...
      buffer.withUnsafeReadableBytesWithStorageManagement {
        ptr, storageRef in
  ...
  let result = try body(
    UnsafeBufferPointer(start: iovecs.baseAddress!, count: numberOfUsedStorageSlots))
...
private struct PendingStreamWritesState {
  ...
  public mutating func didWrite(itemCount: Int, result writeResult: IOResult<Int>)
...
final class PendingStreamWritesManager: PendingWritesManager {
  ...
  private func triggerScalarFileWrite(_ operation: (CInt, Int, Int) throws -> IOResult<Int>) throws
...
extension PendingWritesManager {
  ...
  ) throws -> (OverallWriteResult, Bool) {
