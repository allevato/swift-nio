...
public final class AcceptBackoffHandler: ChannelDuplexHandler {
  ...
  public init(
    backoffProvider: @escaping (IOError) -> TimeAmount? =
      AcceptBackoffHandler.defaultBackoffProvider
  ) {
  ...
  public func errorCaught(ctx: ChannelHandlerContext, error: Error) {
    if let ioError = error as? IOError {
      if let amount = backoffProvider(ioError) {
        self.nextReadDeadlineNS = TimeAmount.Value(DispatchTime.now().uptimeNanoseconds) +
        amount.nanoseconds
...
public class IdleStateHandler: ChannelDuplexHandler {
  ...
  private func makeReadTimeoutTask(_ ctx: ChannelHandlerContext, _ timeout: TimeAmount)
  -> (() -> Void) {
    return {
      ...
      if self.reading {
        self.scheduledReaderTask = ctx.eventLoop.scheduleTask(
          in: timeout,
          self.makeReadTimeoutTask(ctx, timeout))
        return
      }

      let diff =
        TimeAmount.Value(DispatchTime.now().uptimeNanoseconds) - TimeAmount.Value(
          self.lastReadTime.uptimeNanoseconds)
      if diff >= timeout.nanoseconds {
        ...
        self.scheduledReaderTask = ctx.eventLoop.scheduleTask(
          in: .nanoseconds(timeout.nanoseconds - diff),
          self.makeReadTimeoutTask(ctx, timeout))
      }
    }
  }

  private func makeWriteTimeoutTask(_ ctx: ChannelHandlerContext, _ timeout: TimeAmount)
  -> (() -> Void) {
    return {
      guard self.shouldReschedule(ctx) else {
        return
      ...
      if diff >= timeout.nanoseconds {
        ...
        self.scheduledWriterTask = ctx.eventLoop.scheduleTask(
          in: timeout,
          self.makeWriteTimeoutTask(ctx, timeout))
        ...
        self.scheduledWriterTask = ctx.eventLoop.scheduleTask(
          in: .nanoseconds(TimeAmount.Value(timeout.nanoseconds) - TimeAmount.Value(diff)),
          self.makeWriteTimeoutTask(ctx, timeout))
      }
    }
  }

  private func makeAllTimeoutTask(_ ctx: ChannelHandlerContext, _ timeout: TimeAmount)
  -> (() -> Void) {
    return {
      guard self.shouldReschedule(ctx) else {
        return
      ...
      if self.reading {
        self.scheduledReaderTask = ctx.eventLoop.scheduleTask(
          in: timeout,
          self.makeAllTimeoutTask(ctx, timeout))
        return
      }
      ...
      let diff =
        TimeAmount.Value(DispatchTime.now().uptimeNanoseconds) - TimeAmount.Value(
          (lastRead > lastWrite ? lastRead : lastWrite).uptimeNanoseconds)
      if diff >= timeout.nanoseconds {
        // Reader is idle - set a new timeout and trigger an event through the pipeline
        self.scheduledReaderTask = ctx.eventLoop.scheduleTask(
          in: timeout,
        ...
        // Read occurred before the timeout - set a new timeout with shorter delay.
        self.scheduledReaderTask = ctx.eventLoop.scheduleTask(
          in: .nanoseconds(TimeAmount.Value(timeout.nanoseconds) - diff),
          self.makeAllTimeoutTask(ctx, timeout))
      }
    }
  }
