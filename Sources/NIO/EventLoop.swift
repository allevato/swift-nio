...
internal final class SelectableEventLoop: EventLoop {
  ...
  internal func promiseCreationStoreAdd<T>(
  ...
  internal func promiseCreationStoreRemove<T>(future: EventLoopFuture<T>)
  ...
  public init(thread: Thread) throws {
    ...
    self.iovecs = UnsafeMutableBufferPointer(
      start: self._iovecs,
      count: Socket.writevLimitIOVectors)
    self.storageRefs = UnsafeMutableBufferPointer(
      start: self._storageRefs,
      count: Socket.writevLimitIOVectors)
    ...
    self.addresses = UnsafeMutableBufferPointer(
      start: _addresses,
      count: Socket.writevLimitIOVectors)
  ...
  public func register<C: SelectableChannel>(channel: C) throws {
    ...
    try selector.register(
      ...
      interested: channel.interestedEvent,
      makeRegistration: channel.registrationFor(interested:))
  }
  ...
  public func run() throws {
    ...
    while lifecycleState != .closed {
      ...
      try withAutoReleasePool {
        try selector.whenReady(strategy: currentSelectorStrategy(nextReadyTask: nextReadyTask)) {
          ev in
...
extension SelectableEventLoop: CustomStringConvertible {
  var description: String {
    return self.tasksLock.withLock {
      return
        "SelectableEventLoop { selector = \(self.selector), scheduledTasks = \(self.scheduledTasks.description) }"
    }
  }
}
...
final public class MultiThreadedEventLoopGroup: EventLoopGroup {
  ...
  internal init(threadInitializers: [ThreadInitializer]) {
    var idx = 0
    self.eventLoops = threadInitializers.map {
      initializer in
