...
protocol SockAddrProtocol {
  ...
  mutating func withMutableSockAddr<R>(_ body: (UnsafeMutablePointer<sockaddr>, Int) throws -> R)
  rethrows -> R
}

/// Returns a description for the given address.
internal func descriptionForAddress(family: CInt, bytes: UnsafeRawPointer, length byteCount: Int)
throws -> String {
...
class BaseSocket: Selectable {
  ...
  private func get_addr(
    _ body: (Int32, UnsafeMutablePointer<sockaddr>, UnsafeMutablePointer<socklen_t>) throws -> Void
  ...
  static func makeSocket(protocolFamily: Int32, type: CInt, setNonBlocking: Bool = false) throws
  -> Int32 {
    var sockType = type
    #if os(Linux)
      if setNonBlocking {
    ...
    #endif
    let sock = try Posix.socket(domain: protocolFamily, type: sockType, protocol: 0)
    #if !os(Linux)
      if setNonBlocking {
        do {
    ...
    if protocolFamily == AF_INET6 {
      ...
      do {
        try Posix.setsockopt(
          ...
          optionName: IPV6_V6ONLY,
          optionValue: &zero,
          optionLen: socklen_t(MemoryLayout.size(ofValue: zero)))
      }
  ...
  final func setNonBlocking() throws {
    return try withUnsafeFileDescriptor {
      fd in
  ...
  final func setOption<T>(level: Int32, name: Int32, value: T) throws {
    if level == SocketOptionValue(IPPROTO_TCP) && name == TCP_NODELAY && (
        try? self.localAddress().protocolFamily
      ) == Optional<Int32>.some(Int32(Posix.AF_UNIX))
    {
