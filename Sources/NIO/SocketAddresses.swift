...
public enum SocketAddress: CustomStringConvertible {
  ...
  public var description: String {
    ...
    case .v4(let addr):
      ...
      addressString = try! descriptionForAddress(
        family: AF_INET,
        bytes: &mutAddr,
    ...
    case .unixDomainSocket(let addr):
      ...
      type = "UDS"
      addressString = ""
      port = withUnsafeBytes(of: &address.sun_path) {
        ptr in
...
extension SocketAddress: Equatable {
  public static func == (lhs: SocketAddress, rhs: SocketAddress) -> Bool {
    ...
    case (.v4(let addr1), .v4(let addr2)):
      return
        addr1.address.sin_family == addr2.address.sin_family && addr1.address.sin_port ==
        addr2.address.sin_port && addr1.address.sin_addr.s_addr == addr2.address.sin_addr.s_addr
    case (.v6(let addr1), .v6(let addr2)):
      guard addr1.address.sin6_family == addr2.address.sin6_family && addr1.address.sin6_port ==
        addr2.address.sin6_port && addr1.address.sin6_flowinfo == addr2.address.sin6_flowinfo &&
