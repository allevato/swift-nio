...
extension ByteBuffer {
  ...
  @discardableResult public mutating func write(staticString string: StaticString) -> Int {
  ...
  public mutating func set(staticString string: StaticString, at index: Int) -> Int {
    ...
    return self.set(
      bytes: UnsafeRawBufferPointer(start: string.utf8Start, count: string.utf8CodeUnitCount),
      at: index)
  }
  ...
  ///     - string: The string to write.
  /// - returns: The number of bytes written.
  @discardableResult public mutating func write(string: String) -> Int {
  ...
  mutating func _setStringSlowpath(_ string: String, at index: Int) -> Int {
    ...
    if let written =
          (string + "").utf8.withContiguousStorageIfAvailable({ utf8Bytes in
  ...
  ///     - index: The index for the first serialized byte.
  /// - returns: The number of bytes written.
  @discardableResult @inlinable public mutating func set(string: String, at index: Int) -> Int {
    if let written =
          string.utf8.withContiguousStorageIfAvailable({ utf8Bytes in
            self.set(bytes: utf8Bytes, at: index)
          })
  ...
  public func getString(at index: Int, length: Int) -> String? {
    precondition(index >= 0, "index must not be negative")
    precondition(length >= 0, "length must not be negative")
    return withVeryUnsafeBytes {
      pointer in
      guard index <= pointer.count - length else {
        return nil
      }
      return String(
        decoding: UnsafeRawBufferPointer(rebasing: pointer[index..<(index+length)]),
  ...
  /// - returns: The number of bytes written.
  @discardableResult public mutating func write(dispatchData: DispatchData) -> Int {
  ...
  ///     - index: The index for the first serialized byte.
  /// - returns: The number of bytes written.
  @discardableResult public mutating func set(dispatchData: DispatchData, at index: Int) -> Int {
    let allBytesCount = dispatchData.count
    self.reserveCapacity(index + allBytesCount)
    self.withVeryUnsafeMutableBytes {
      destCompleteStorage in
  ...
  public func getDispatchData(at index: Int, length: Int) -> DispatchData? {
    precondition(index >= 0, "index must not be negative")
    precondition(length >= 0, "length must not be negative")
    return self.withVeryUnsafeBytes {
      pointer in
      guard index <= pointer.count - length else {
        return nil
      }
  ...
  /// - returns: The number of bytes written.
  @discardableResult public mutating func set(buffer: ByteBuffer, at index: Int) -> Int {
  ...
  @discardableResult public mutating func write(buffer: inout ByteBuffer) -> Int {
  ...
  /// - returns: The number of bytes written or `bytes.count`.
  @discardableResult @inlinable public mutating func write(bytes: UnsafeRawBufferPointer) -> Int {
    let written = self.set(bytes: bytes, at: self.writerIndex)
    self._moveWriterIndex(forwardBy: written)
    return written
  }
