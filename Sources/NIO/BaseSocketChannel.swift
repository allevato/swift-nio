...
class BaseSocketChannel<T: BaseSocket>: SelectableChannel, ChannelCore {
  ...
  private let addressesCached: AtomicBox<Box<(local: SocketAddress?, remote: SocketAddress?)>> =
    AtomicBox(value: Box((local: nil, remote: nil)))
  ...
  init(
    ...
    self.addressesCached.store(
      Box((local: try? socket.localAddress(), remote: try? socket.remoteAddress())))
  }
  ...
  public final func setOption<T: ChannelOption>(option: T, value: T.OptionType)
  ...
  @discardableResult private final func readable0() -> ReadStreamState {
    self.eventLoop.assertInEventLoop()
    assert(self.lifecycleManager.isActive)
    ...
    do {
      ...
      if let channelErr = err as? ChannelError, channelErr == ChannelError.eof {
        ...
        if self.lifecycleManager.isActive, try! getOption0(
  ...
  internal final func updateCachedAddressesFromSocket(
    updateLocal: Bool = true,
  ...
  private final func safeReregister(interested: SelectorEventSet) {
    ...
    guard self.isOpen else {
      assert(
        self.interestedEvent == .reset,
        "interestedEvent=\(self.interestedEvent) event though we're closed")
      return
    }
