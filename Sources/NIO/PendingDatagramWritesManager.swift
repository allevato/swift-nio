...
private func doPendingDatagramWriteVectorOperation(
  ...
  assert(
    msgs.count >= Socket.writevLimitIOVectors,
    "Insufficiently sized buffer for a maximal sendmmsg")
  ...
  for p in pending.flushedWrites {
    ...
    p.data.withUnsafeReadableBytesWithStorageManagement {
      ptr, storageRef in
      ...
      var msg = msghdr(
        msg_name: addresses.baseAddress! + c,
...
private struct PendingDatagramWritesState {
  ...
  private mutating func subtractOutstanding(bytes: Int) {
    assert(
      self.bytes >= bytes,
      "allegedly written more bytes (\(bytes)) than outstanding (\(self.bytes))")
  ...
  public mutating func recoverableError(_ error: Error)
  -> (DatagramWritePromiseFiller?, OneWriteOperationResult) {
    ...
    let result: OneWriteOperationResult =
      self.pendingWrites.hasMark ? .writtenPartially : .writtenCompletely
  ...
  ///     everything or not.
  private mutating func didVectorWrite(written: Int, messages: UnsafeMutableBufferPointer<MMsgHdr>)
  -> (DatagramWritePromiseFiller?, OneWriteOperationResult) {
    ...
    let writes =
      messages[messages.startIndex...messages.index(messages.startIndex, offsetBy: written - 1)]
  ...
  ///     everything or not.
  private mutating func didScalarWrite(written: Int)
  -> (DatagramWritePromiseFiller?, OneWriteOperationResult) {
    precondition(
      written <= self.pendingWrites[0].data.readableBytes,
...
final class PendingDatagramWritesManager: PendingWritesManager {
  ...
  internal var waterMark: WriteBufferWaterMark = WriteBufferWaterMark(
    low: 32 * 1024,
    high: 64 * 1024)
  ...
  func add(envelope: AddressedEnvelope<ByteBuffer>, promise: EventLoopPromise<Void>?) -> Bool {
    ...
    if self.state.bytes > waterMark.high && channelWritabilityFlag.compareAndExchange(
        expected: true,
        desired: false)
    {
  ...
  func triggerAppropriateWriteOperations(
    scalarWriteOperation: (UnsafeRawBufferPointer, UnsafePointer<sockaddr>, socklen_t) throws ->
    IOResult<Int>,
    vectorWriteOperation: (UnsafeMutableBufferPointer<MMsgHdr>) throws -> IOResult<Int>
  ) throws -> (writeResult: OverallWriteResult, writable: Bool) {
  ...
  private func didWrite(_ data: IOResult<Int>, messages: UnsafeMutableBufferPointer<MMsgHdr>?)
    ...
    do {
      let writeResult = try pending.address.withSockAddr { (addrPtr, addrSize) in
        try pending.data.withUnsafeReadableBytes {
          try scalarWriteOperation($0, addrPtr, socklen_t(addrSize))
        }
      }
