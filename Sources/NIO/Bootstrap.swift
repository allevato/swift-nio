...
public final class ServerBootstrap {
  ...
  public func serverChannelInitializer(_ initializer: @escaping (Channel) -> EventLoopFuture<Void>)
  -> Self {
  ...
  public func withBoundSocket(descriptor: CInt) -> EventLoopFuture<Channel> {
    func makeChannel(_ eventLoop: SelectableEventLoop, _ childEventLoopGroup: EventLoopGroup) throws
    -> ServerSocketChannel {
    ...
    return bind0(makeServerChannel: makeChannel) {
  ...
  private func bind0(
    makeServerChannel: (_ eventLoop: SelectableEventLoop, _ childGroup: EventLoopGroup) throws ->
    ServerSocketChannel,
  ...
  private class AcceptHandler: ChannelInboundHandler {
    ...
    func channelRead(ctx: ChannelHandlerContext, data: NIOAny) {
      ...
      let childChannelInit =
        self.childChannelInit ?? { (_: Channel) in
          childEventLoop.makeSucceededFuture(result: ())
        }

      @inline(__always)
      func setupChildChannel() -> EventLoopFuture<Void> {
        return self.childChannelOptions.applyAll(channel: accepted).then {
...
extension Channel {
  private func registerAndDoSynchronously(_ body: @escaping (Channel) -> EventLoopFuture<Void>)
...
public final class ClientBootstrap {
  ...
  public func connect(host: String, port: Int) -> EventLoopFuture<Channel> {
    ...
    let connector = HappyEyeballsConnector(
      ...
      resolver ?? GetaddrinfoResolver(
        loop: loop,
        aiSocktype: Posix.SOCK_STREAM,
        aiProtocol: Posix.IPPROTO_TCP),
      loop: loop,
      host: host,
      port: port,
      connectTimeout: self.connectTimeout) { eventLoop, protocolFamily in
      return self.execute(eventLoop: eventLoop, protocolFamily: protocolFamily) {
        $0.eventLoop.makeSucceededFuture(result: ())
      }
    }
  ...
  public func connect(to address: SocketAddress) -> EventLoopFuture<Channel> {
    return execute(eventLoop: group.next(), protocolFamily: address.protocolFamily) {
      channel in
  ...
  public func withConnectedSocket(descriptor: CInt) -> EventLoopFuture<Channel> {
    ...
    let channelInitializer =
      self.channelInitializer ?? { _ in
        eventLoop.makeSucceededFuture(result: ())
      }
    let channel: SocketChannel
    do {
      channel = try SocketChannel(
...
public final class DatagramBootstrap {
  ...
  public func withBoundSocket(descriptor: CInt) -> EventLoopFuture<Channel> {
    ...
    return bind0(makeChannel: makeChannel) {
      (eventLoop, channel) in
      let promise = eventLoop.makePromise(of: Void.self)
      channel.registerAlreadyConfigured0(promise: promise)
      return promise.futureResult
    }
  ...
  private func bind0(_ makeSocketAddress: () throws -> SocketAddress) -> EventLoopFuture<Channel> {
    ...
    func makeChannel(_ eventLoop: SelectableEventLoop) throws -> DatagramChannel {
      return try DatagramChannel(eventLoop: eventLoop, protocolFamily: address.protocolFamily)
    }
  ...
  private func bind0(
    ...
    _ registerAndBind: @escaping (EventLoop, DatagramChannel) -> EventLoopFuture<Void>
  ) -> EventLoopFuture<Channel> {
    let eventLoop = self.group.next()
    let channelInitializer =
      self.channelInitializer ?? { _ in
...
internal struct ChannelOptionStorage {
  ...
  mutating func put<K: ChannelOption>(key: K, value: K.OptionType) {
    if K.self == SocketOption.self {
      ...
      return self.put(key: key, value: value) { _, _ in
        true
      }
    }
  }

  mutating func put<K: ChannelOption>(
    key: K,
