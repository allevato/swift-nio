...
internal struct Heap<T: Comparable> {
  ...
  private static func heapRootify(
    storage: inout ContiguousArray<T>,
    compare: (T, T) -> Bool,
    index: Int,
    key: T
  ) {
  ...
  @discardableResult public mutating func remove(value: T) -> Bool {
  ...
  @discardableResult private mutating func remove(index: Int) -> T? {
    ...
    if self.storage.count == 1 || self.storage[index] == self.storage[self.storage.count - 1] {
      ...
      Heap<T>.heapRootify(
        ...
        index: index,
        key: self.storage[self.storage.count - 1])
      self.storage.removeLast()
    }
    else {
...
extension Heap: CustomDebugStringConvertible {
  public var debugDescription: String {
    ...
    let paddedDescs = descriptions.map {
