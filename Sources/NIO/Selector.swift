...
extension timespec {
  private init(timeAmount amount: TimeAmount) {
...
extension Optional {
  private func withUnsafeOptionalPointer<T>(_ body: (UnsafePointer<Wrapped>?) throws -> T) rethrows
  -> T {
    if var this = self {
...
extension KQueueEventFilterSet {
  ...
  func calculateKQueueFilterSetChanges(previousKQueueFilterSet: KQueueEventFilterSet,
      ...
      try withUnsafeMutableBytes(of: &keventsHopefullyOnStack) {
  rawPtr in
...
final class Selector<R: Registration> {
  ...
  init() throws {
    ...
    self.lifecycleState = .closed
      ...
      eventfd = try EventFd.eventfd(
        initval: 0,
        flags: Int32(EventFd.EFD_CLOEXEC | EventFd.EFD_NONBLOCK))
      timerfd = try TimerFd.timerfd_create(
  ...
  deinit {
    assert(
      self.lifecycleState == .closed,
      "Selector \(self.lifecycleState) (expected .closed) on deinit")
    Selector.deallocateEventsArray(events: events, capacity: eventsCapacity)
...
//         wakeup calls as there are no references to this selector left. 💁
  ...
  func register<S: Selectable>(
    ...
    makeRegistration: (SelectorEventSet) -> R
  ) throws {
    assert(interested.contains(.reset))
    guard self.lifecycleState == .open else {
      throw IOError(
        errnoCode: EBADF,
        reason: "can't register on selector as it's \(self.lifecycleState).")
    }
  ...
  func reregister<S: Selectable>(selectable: S, interested: SelectorEventSet) throws {
    guard self.lifecycleState == .open else {
      throw IOError(
        errnoCode: EBADF,
        reason: "can't re-register on selector as it's \(self.lifecycleState).")
    }
    assert(
      interested.contains(.reset),
      "must register for at least .reset but tried registering for \(interested)")
    try selectable.withUnsafeFileDescriptor {
      fd in
  ...
  func deregister<S: Selectable>(selectable: S) throws {
    guard self.lifecycleState == .open else {
      throw IOError(
        errnoCode: EBADF,
        reason: "can't deregister from selector as it's \(self.lifecycleState).")
    }
  ...
  func whenReady(strategy: SelectorStrategy, _ body: (SelectorEvent<R>) throws -> Void) throws {
    guard self.lifecycleState == .open else {
      throw IOError(
        errnoCode: EBADF,
        reason: "can't call whenReady for selector as it's \(self.lifecycleState).")
    }
  ...
  public func close() throws {
    guard self.lifecycleState == .open else {
      throw IOError(
        errnoCode: EBADF,
        reason: "can't close selector as it's \(self.lifecycleState).")
    }
    self.lifecycleState = .closed
  ...
  func closeGently(eventLoop: EventLoop) -> EventLoopFuture<Void> {
    guard self.lifecycleState == .open else {
      return eventLoop.makeFailedFuture(
        error: IOError(
          errnoCode: EBADF,
          reason: "can't close selector gently as it's \(self.lifecycleState)."))
    }
