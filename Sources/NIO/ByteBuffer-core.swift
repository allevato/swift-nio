...
let sysMalloc: @convention(c)
(size_t) -> UnsafeMutableRawPointer? = malloc
let sysRealloc: @convention(c)
(UnsafeMutableRawPointer?, size_t) -> UnsafeMutableRawPointer? = realloc
let sysFree: @convention(c)
(UnsafeMutableRawPointer?) -> Void = free
...
@usableFromInline struct _ByteBufferSlice {
...
public struct ByteBufferAllocator {
  ...
  internal init(
    hookedMalloc: @escaping @convention(c)
    (size_t) -> UnsafeMutableRawPointer?,
    hookedRealloc: @escaping @convention(c)
    (UnsafeMutableRawPointer?, size_t) -> UnsafeMutableRawPointer?,
    hookedFree: @escaping @convention(c)
    (UnsafeMutableRawPointer?) -> Void,
    hookedMemcpy: @escaping @convention(c)
    (UnsafeMutableRawPointer, UnsafeRawPointer, size_t) -> Void
  ) {
  ...
  internal let malloc: @convention(c)
  (size_t) -> UnsafeMutableRawPointer?
  internal let realloc: @convention(c)
  (UnsafeMutableRawPointer?, size_t) -> UnsafeMutableRawPointer?
  internal let free: @convention(c)
  (UnsafeMutableRawPointer?) -> Void
  internal let memcpy: @convention(c)
  (UnsafeMutableRawPointer, UnsafeRawPointer, size_t) -> Void
  }
...
public struct ByteBuffer {
  ...
  @usableFromInline final class _Storage {
    ...
    private static func allocateAndPrepareRawMemory(bytes: _Capacity, allocator: Allocator)
    ...
    private func allocateStorage(capacity: _Capacity) -> _Storage {
      ...
      return _Storage(
        bytesNoCopy: _Storage.allocateAndPrepareRawMemory(
          bytes: newCapacity,
    ...
    public func reallocSlice(_ slice: Range<ByteBuffer._Index>, capacity: _Capacity) -> _Storage {
      ...
      self.allocator.memcpy(
        new.bytes,
        self.bytes.advanced(by: Int(slice.lowerBound)),
        size_t(slice.count))
      return new
    }
    ...
    public static func reallocated(minimumCapacity: _Capacity, allocator: Allocator) -> _Storage {
      ...
      return _Storage(
        bytesNoCopy: _Storage.allocateAndPrepareRawMemory(bytes: newCapacity, allocator: allocator),
        capacity: newCapacity,
        allocator: allocator)
    }
  ...
  private mutating func _copyStorageAndRebase(capacity: _Capacity, resetIndices: Bool = false) {
    ...
    let newSlice =
      storageRebaseAmount..<min(
        storageRebaseAmount + _toCapacity(self._slice.count),
        self._slice.upperBound,
        storageRebaseAmount + capacity)
  ...
  mutating func _copyStorageAndRebase(extraCapacity: _Capacity = 0, resetIndices: Bool = false) {
    self._copyStorageAndRebase(
      capacity: _toCapacity(self._slice.count) + extraCapacity,
  ...
  @inlinable mutating func _moveReaderIndex(to newIndex: _Index) {
    assert(newIndex >= 0 && newIndex <= writerIndex)
    self._readerIndex = newIndex
  }

  @inlinable mutating func _moveReaderIndex(forwardBy offset: Int) {
    let newIndex = self._readerIndex + _toIndex(offset)
    self._moveReaderIndex(to: newIndex)
  }

  @inlinable mutating func _moveWriterIndex(to newIndex: _Index) {
    assert(newIndex >= 0 && newIndex <= _toCapacity(self._slice.count))
    self._writerIndex = newIndex
  }

  @inlinable mutating func _moveWriterIndex(forwardBy offset: Int) {
    let newIndex = self._writerIndex + _toIndex(offset)
    self._moveWriterIndex(to: newIndex)
  }

  @inlinable mutating func _set(bytes: UnsafeRawBufferPointer, at index: _Index) -> _Capacity {
  ...
  where Bytes.Element == UInt8 {
    ...
    if !isKnownUniquelyReferenced(&self._storage) {
      let extraCapacity =
        newPastEndIndex > self._slice.upperBound ? newPastEndIndex - self._slice.upperBound : 0
      self._copyStorageAndRebase(extraCapacity: extraCapacity)
    }

    var base = ensureCapacityAndReturnStorageBase(capacity: underestimatedByteCount)
    var (iterator, idx) =
      UnsafeMutableBufferPointer(start: base, count: underestimatedByteCount).initialize(
      from: bytes)
  ...
  @inlinable
  mutating func _set<Bytes: Sequence>(bytes: Bytes, at index: _Index) -> _Capacity
  where Bytes.Element == UInt8 {
    if let written =
          bytes.withContiguousStorageIfAvailable({ bytes in
  ...
  @usableFromInline mutating func _copyStorageAndRebaseIfNeeded() {
    if !isKnownUniquelyReferenced(&self._storage) {
      self._copyStorageAndRebase()
    }
  }
  ...
  public mutating func withUnsafeMutableReadableBytes<T>(
    ...
    return try body(
      .init(rebasing: self._slicedStorageBuffer[readerIndex..<readerIndex + self.readableBytes]))
  }
...
extension ByteBuffer {
  ...
  @discardableResult
  @inlinable
  public mutating func set<Bytes: Sequence>(bytes: Bytes, at index: Int) -> Int
  where Bytes.Element == UInt8 {
